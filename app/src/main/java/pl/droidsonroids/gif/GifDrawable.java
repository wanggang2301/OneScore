...
public class GifDrawable extends Drawable implements Animatable,
	...
	public GifDrawable(InputStream stream) throws IOException {
		this(GifInfoHandle.openMarkableInputStream(stream, false), null, null,
				true);
	}
	...
	public GifDrawable(AssetFileDescriptor afd) throws IOException {
		this(GifInfoHandle.openAssetFileDescriptor(afd, false), null, null,
				true);
	}
	...
	public GifDrawable(ByteBuffer buffer) throws IOException {
		this(GifInfoHandle.openDirectByteBuffer(buffer, false), null, null,
				true);
	}
		...
		if (lastFrameRemainder >= 0) {
			if (mIsRenderingTriggeredOnDraw) {
				...
				while (mExecutor.getQueue().remove(mRenderTask))
					;
				mExecutor.schedule(mRenderTask, lastFrameRemainder,
	...
	public String toString() {
		return String.format(Locale.US,
				...
				mNativeInfoHandle.width, mNativeInfoHandle.height,
				mNativeInfoHandle.frameCount,
	 ...
	 * animation if there is no loop) then animation will be sought to the end.<br>
...
//			byteCount += mBuffer.getAllocationByteCount();
			byteCount += mBuffer.getByteCount();
		} else {
	...
	public void getPixels(int[] pixels) {
		mBuffer.getPixels(pixels, 0, mNativeInfoHandle.width, 0, 0,
				mNativeInfoHandle.width, mNativeInfoHandle.height);
	}
	...
	public void draw(Canvas canvas) {
		...
		if (mIsRenderingTriggeredOnDraw && mIsRunning
			...
			final long renderDelay = Math.max(0, mNextFrameRenderTime
...
//	private AnimationListener alListener;
...
//	public  void setAnim() {
//		alListener.onAnimationCompleted();
//	}
