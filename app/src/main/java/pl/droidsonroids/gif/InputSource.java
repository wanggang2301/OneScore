...
public abstract class InputSource {
	...
	final GifDrawable build(GifDrawable oldDrawable,
							ScheduledThreadPoolExecutor executor,
							boolean isRenderingAlwaysEnabled) throws IOException {
		return new GifDrawable(open(), oldDrawable, executor,
	...
	/**
	 * Indicates whether the content of this source is opaque. GIF that is known
	 * to be opaque can take a faster drawing case than non-opaque one. See
	 * {@link GifTextureView#setOpaque(boolean)} for more information.<br>
	 * Currently it is used only by {@link GifTextureView}, not by
	 * {@link GifDrawable}
	 *
	 * @param isOpaque
	 *            whether the content of this source is opaque
	...
	public static final class DirectByteBufferSource extends InputSource {
		...
		/**
		 * Constructs new source. Buffer can be larger than size of the GIF
		 * data. Bytes beyond GIF terminator are not accessed.
		 *
		 * @param byteBuffer
		 *            source buffer, must be direct
		 */
	...
	public static final class ByteArraySource extends InputSource {
		...
		/**
		 * Constructs new source. Array can be larger than size of the GIF data.
		 * Bytes beyond GIF terminator are not accessed.
		 *
		 * @param bytes
		 *            source array
		 */
	...
	public static final class AssetSource extends InputSource {
		...
		@Override
		GifInfoHandle open() throws IOException {
			return GifInfoHandle.openAssetFileDescriptor(
					mAssetManager.openFd(mAssetName), false);
		}
	}
	...
	public static final class InputStreamSource extends InputSource {
		 ...
		 *
		 * @param inputStream
		 *            source input stream, it must support marking
		 */
	...
	public static class ResourcesSource extends InputSource {
		...
		GifInfoHandle open() throws IOException {
			return GifInfoHandle.openAssetFileDescriptor(
					mResources.openRawResourceFd(mResourceId), false);
		}
	}
	...
	public static class AssetFileDescriptorSource extends InputSource {
		...
		@Override
		GifInfoHandle open() throws IOException {
			return GifInfoHandle.openAssetFileDescriptor(mAssetFileDescriptor,
					false);
		}
	}
