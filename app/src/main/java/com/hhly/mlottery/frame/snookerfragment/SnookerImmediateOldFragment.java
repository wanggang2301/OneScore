 ...
 * mail：yixq@13322.com
 * describe: snooker 即时列表页Fragment
 */

public class SnookerImmediateOldFragment extends BaseWebSocketFragment implements SwipeRefreshLayout.OnRefreshListener, View.OnClickListener{
    ...
    private List<SnookerWaterfallMatchesBean> mWaterfallMatchesList = new ArrayList<>();
...
//        setWebSocketUri("ws://192.168.10.242:61634");
    ...
    public static SnookerImmediateOldFragment newInstance() {

        Bundle args = new Bundle();

        SnookerImmediateOldFragment fragment = new SnookerImmediateOldFragment();
...
//        return super.onCreateView(inflater, container, savedInstanceState);
        ...
        mView = inflater.inflate(R.layout.snooker_list_activity, container, false);
...
//        initData();
    ...
    private void initView() {
        ...
        TextView mTitle = (TextView) mView.findViewById(R.id.public_txt_title);
        ...
        mFooterView = LayoutInflater.from(mContext).inflate(R.layout.view_load_more, mRecyclerView, false);
        ...
        //隐藏公共头部多余图标
        mFilterImage = (ImageView) mView.findViewById(R.id.public_btn_filter);
        mFilterImage.setVisibility(View.GONE);

        mBackImage = (ImageView) mView.findViewById(R.id.public_img_back);
        mBackImage.setVisibility(View.GONE);
//        mBackImage.setOnClickListener(this);

        mSettingImage = (ImageView) mView.findViewById(R.id.public_btn_set);
    ...
    private void testInitData(){
        setStatus(SHOW_STATUS_LOADING);
        List<SnookerLeaguesBean> testData = testFirstData(3);
        //数据加载
        setData(currentAllData ,testData);
        /**
         * 区分上拉  下拉 的日期
         */
        upDateList = new ArrayList<>();
        downDateList = new ArrayList<>();
        SnookerWaterfallMatchesBean waterData = new SnookerWaterfallMatchesBean();
        waterData.setDate("2017-02-15");
        waterData.setCount("1");
        waterData.setLeagueId("125358");
        mWaterfallMatchesList.add(waterData);
        ...
        if (mWaterfallMatchesList.size() != 0) {
            ...
            for (SnookerWaterfallMatchesBean date : mWaterfallMatchesList) {
                 ...
                 * 获得下拉的所有日期
                 */
                if (!date.getDate().equals(testData.get(0).getDate())) {
                    if (isMinCurrentDate) {
                 ...
                 * 获得上拉所有数据
                 */
                if (!date.getDate().equals(testData.get(testData.size()-1).getDate())) {
                    if (isMaxCurrentDate) {
                        upDateList.add(date);
                    }
                }else{
...
//                        updateAdapter();
    ...
    private void setData(List<SnookerMatchesBean> listData, List<SnookerLeaguesBean> dataBean) {
        ...
        if (mAdapter == null) {
            mAdapter = new SnookerRecyclerAdapter(mContext, listData);
...
//                        addData(parameter , type);
    ...
    private void testAddData(){
        List<SnookerLeaguesBean> testData = testFirstData(1);
        //数据加载
        setData(currentAllData ,testData);
        upLoadNum++;
        updateAdapter();
...
//                updateAdapter();
                Toast.makeText(mContext, "yxqAAAAAAAA" + currentAllData.size(), Toast.LENGTH_SHORT).show();
            }
        }, new VolleyContentFast.ResponseErrorListener() {
            @Override
            public void onErrorResponse(VolleyContentFast.VolleyException exception) {
                Toast.makeText(mContext, "yxqBBBBBBBB" + currentAllData.size(), Toast.LENGTH_SHORT).show();
            }
...
////                finish();   cyqhqrvuixtibdhg
//                break;
            case R.id.public_btn_set:

                Intent intent = new Intent(mContext, SnookerSettingActivity.class);
...
//        L.d("yxq=====>updataItemData()调用 ：","data.getStatus = "+data.getStatus());
    ...
    private List<SnookerLeaguesBean> testFirstData(int index){
        List<SnookerLeaguesBean> data = new ArrayList<>();

        for (int i = 0; i < index; i++) {
            SnookerLeaguesBean leaguesbean = new SnookerLeaguesBean();
            leaguesbean.setLeaguesName("全球逗逼总决赛");
            leaguesbean.setDate("2017-02-14 * " + i);
            leaguesbean.setLeaguesId("125358");
            leaguesbean.setMatches(testData(5));
            data.add(leaguesbean);
        }
        return data;
    }
    private List<SnookerMatchesBean> testData(int num){
        List<SnookerMatchesBean> mData = new ArrayList<>();
        for (int k = 0; k < num; k++) {
            SnookerMatchesBean data = new SnookerMatchesBean();
            data.setItemType(3);
            data.setHomeTeam("阿诺德·施瓦辛格");
            data.setGuestTeam("西尔威斯特·史泰龙");
            data.setStatus("1");
            mData.add(data);
        }
        return mData;
