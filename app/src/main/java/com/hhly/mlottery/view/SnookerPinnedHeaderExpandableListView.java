...
/**
 * Created by yuely198 on 2017/2/16.
 */

public class SnookerPinnedHeaderExpandableListView extends ExpandableListView implements AbsListView.OnScrollListener,ExpandableListView.OnGroupClickListener {
    @Override
    public void setChildDivider(Drawable childDivider) {
        super.setChildDivider(childDivider);
    }

    public SnookerPinnedHeaderExpandableListView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        registerListener();
    }

    public SnookerPinnedHeaderExpandableListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        registerListener();
    }

    public SnookerPinnedHeaderExpandableListView(Context context) {
        super(context);
        registerListener();
    }
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (x <= mHeaderViewWidth && y <= mHeaderViewHeight
                        ...
                        if (mHeaderView != null) {
                            headerViewClick();//暂时注释，需要点击头部的时候打开
                        }
    ...
    public boolean onGroupClick(ExpandableListView parent,View v,int groupPosition,long id) {
        if (mAdapter.getGroupClickStatus(groupPosition) == 0) {
            mAdapter.setGroupClickStatus(groupPosition, 1);//暂时注释，需要点击头部的时候打开
            parent.expandGroup(groupPosition);
            //Header自动置顶
//			parent.setSelectedGroup(groupPosition);

        } else if (mAdapter.getGroupClickStatus(groupPosition) == 1) {
            mAdapter.setGroupClickStatus(groupPosition, 0);//暂时注释，需要点击头部的时候打开
